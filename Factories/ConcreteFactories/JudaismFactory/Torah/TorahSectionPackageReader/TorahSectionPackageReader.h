...
namespace scrptm {
    ...
    class TorahSectionPackageReader {
        ...
        static std::unique_ptr<const TorahVariationFile> readVariationFile(std::unique_ptr<const json> &&json) ;
        static std::unique_ptr<const TorahSectionWordFile> readWordFile(std::unique_ptr<const xml> &&xml);

        static void validateFileContent(const json &usualFileJson, const json &simplifiedJsonFile, const json &noVowelJsonFile,
    ...
    public:
        ...
        [[nodiscard]] std::vector<std::unique_ptr<const TorahSectionPackage> > read();
    };
} // scrptm
